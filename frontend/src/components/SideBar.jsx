      ...
      <div className={"w-full h-full grid grid-cols-[48px,1fr]"}>
        ...
        <div className={"w-full bg-primary border-r-2 border-r-secondary"}>
          ...
          <div className={"h-[calc(100vh-65px)] overflow-x-hidden overflow-y-auto scrollbar"}>
            {allUsers?.length ? allUsers?.map((conv) => {
                return (
                    <Link to={"/" + conv?.userDetails?._id}
                      ...
                      <div className={"ml-2"}>
                        <Avatar imageUrl={conv?.userDetails?.profilePic}
                      ...
                      {Boolean(conv?.countUnseenMsg) && (
                          <p className={"text-sx w-6 h-6 flex justify-center items-center ml-auto p-1 bg-primary text-white font-semibold rounded-full"}>{conv?.countUnseenMsg}</p>
                      )}
                    </Link>
                );
              })
              : (
                  <div className="mt-12">
                    <div
                        className="flex justify-center items-center my-4 text-slate-500">
                      <ArrowUpLeft size={50}/>
                    </div>
